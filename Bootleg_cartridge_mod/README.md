...
- Variant 1 is populated with two MX26L6420 chips for C ROMs. This chip is just impossible to reprogram with el cheapo programmers like the GQ-4x4 or the XGECU T48. You are cheap so you will just remove them with hot air and trash them. On the other hand, the nearly pin compatible MX29LV320 is quite well supported and common. You of course need a SOP 44 to DIP 44 adapter to flash it. You just need to cut trace to pin 1 and place a bodge wire between pin 1 and VCC to force the chip in read mode. Flash with the ROMS interlaced by byte (two ROMs, one chip), solder back, that's it.
