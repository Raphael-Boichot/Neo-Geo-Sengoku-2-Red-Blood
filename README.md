...
The project proposes a multi-OS, highly reusable workflow to uncensor Sengoku 2 on Neo Geo as well as IPS patches for all know versions of the game.

This hack has only one purpose: turn the blood red. Nothing else.
...
The regular hack itself has been validated with a complete walktrough in MAME 0.289. You must use MAME in [command line](/Tools/Run_sengoku2.ps1) to force it to run a game not listed in its own database. I plan to build a cartridge version for myself just to be sure that everything is OK on genuine MVS slots.
