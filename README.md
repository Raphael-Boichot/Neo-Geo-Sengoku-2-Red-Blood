...
Prerequites:
- Matlab is installed (or GNU Octave, but execution will be quite slow)
...
Starting confident after these little surprises, I initially though hacking individual files of the NGCD version contained in track 1 and rebuilding an iso from any dedicated tool would be enough. As far as I can tell, it does not work. Even the trusty [neogeodev dedicated page](https://wiki.neogeodev.org/index.php/Making_an_ISO_file) was finally not of any help. Any tool gives me an .iso container too small that makes the Neo Geo CD crash. I was clearly missing something like alignement tricks / error correction / padding. Apparently graving it to a real CD and dump it back would fix the issue but you have to edit the . CUE file too and I do not want wasting time and CDs for testing.
